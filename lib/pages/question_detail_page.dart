...
class _QuestionDetailPageState extends State<QuestionDetailPage> with TickerProviderStateMixin {
  ...
  Future<void> _ensureQuizSession() async {
    if (_quizSessionId != null) return;
    try {
      final res = await _apiService.startQuiz(
        sourceId: widget.questionId,
        sourceType: 'question',
      );
      setState(() {
        _quizSessionId = (res['sessionId'] ?? res['data']?['sessionId'] ?? res['id'])?.toString();
      });
      if (_quizSessionId == null) {
        throw Exception('Không tạo được phiên làm bài');
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Lỗi tạo phiên làm bài: $e'),
  ...
  // Removed duplicate legacy select/submit implementations to keep quiz-integrated versions
  ...
  Future<void> _loadQuestionDetail() async {
    ...
    try {
      ...
      // For now, we'll use the question data passed from the list
      // In the future, we can add a specific API endpoint for single question
      if (widget.questionData != null) {
        setState(() {
          _question = widget.questionData;
          _isLoading = false;
  ...
  Widget _buildDiscussionTab(bool isDesktop) {
    return Column(
      children: [
        // Auth Status Widget
        Padding(
                  ...
                  ? Center(
                      child: Text(
                        _isAuthenticated
                            ...
                            : 'Đăng nhập để xem và thêm bình luận.',
                        style: TextStyle(
                          fontSize: isDesktop ? 16 : 14,
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            child: Text(
                  comment['userName']?.toString().substring(0, 1).toUpperCase() ?? '?',
              style: TextStyle(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      comment['userName']?.toString() ?? 'Người dùng ẩn danh',
                      style: TextStyle(
                fontWeight: FontWeight.bold,
                        fontSize: isDesktop ? 14 : 12,
                        color: const Color(0xFF1F2937),
          ...
          SizedBox(height: isDesktop ? 12 : 8),
          Text(
            comment['content']?.toString() ?? 'Không có nội dung bình luận',
            style: TextStyle(
              fontSize: isDesktop ? 14 : 12,
              color: const Color(0xFF374151),
              height: 1.5,
